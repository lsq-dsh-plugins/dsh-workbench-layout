...
- 仓库工具栏支持查看和切换本地/远程分支；可以从当前提交或指定引用创建分支、重命名当前分支，以及安全删除已合并的非当前本地分支。同时显示上游分支的拉取/推送计数，并提供显式的 Fetch、快进 Pull、Push、发布当前分支和先拉后推的 Sync。
...
- 分支和远程操作只由显式按钮触发，并禁用终端凭据提示；删除分支使用 Git 的已合并安全检查，Pull/Sync 使用快进模式，失败时不会自动合并、暂存或覆盖更改。
