...
- 右栏继续使用 DSH 原生聊天、输入框、任务状态与交互流程；工作台展开时，插件标记实际的原生会话根节点并使其与左栏使用同一套官方主题表面色，官方输入卡片及其全部子节点仍不受覆盖。收起中栏会同时释放列重排和主题覆盖，使聊天恢复官方中栏位置与原始颜色。窄栏下错误码会移到完整宽度的错误正文下方，输入工具栏仍保持单行，仅收紧间距、隐藏推理等级文字和权限/模型选择器内的下拉箭头，并省略过长模型名；箭头隐藏后，权限和模型按钮使用相同的左右内边距，模型选择器按内容宽度收缩，不再吞掉工具栏剩余空间。官方 Session-log 胶囊按钮只在此窄栏宽度下收成单个带无障碍名称的下载图标。官方模型菜单打开时仍由原 React 组件持有，但使用视口浮层坐标，避免被中栏裁切。
