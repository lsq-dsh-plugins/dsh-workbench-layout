...
- 左栏顶部通过紧凑的“会话 / 文件 / Git / 终端”控件切换；收起态严格采用 DSH 官方的 36px 圆形点击区与 12px 纵向节奏，展开态继续使用紧凑文字标签。“会话”使用 DSH 官方的列表气泡图标，与官方“新建会话”图形明确区分，并会恢复原生工作区与会话列表。展开态不再重复显示“新会话”长按钮；收起态只在“会话”模式保留原生新建会话行为，同时在浏览器绘制前同步状态并统一为 36px 正圆，切换时不会晚一帧出现或呈现方形底板。
