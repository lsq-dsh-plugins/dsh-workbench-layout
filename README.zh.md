...
- 收起栏的中段区域随模式切换：文件模式提供新建文件、新建文件夹和刷新；Git 模式提供更改/提交图切换、同步和刷新；终端模式提供新建终端及已有终端选择。收起态选中项沿用 DSH 的 active 表面色与信息蓝；文件树和新建入口使用明确的圆角折角文件图形，文件夹及叠加加号直接复用 DSH 官方图标。需要输入或完整状态的操作会先展开左栏，再通过一次性命令交给对应面板，避免重复执行。
