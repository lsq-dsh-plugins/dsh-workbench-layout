...
- Git uses fixed argv without a shell. A commit runs only after an explicit Commit action. Discard actions are limited to the working tree, require confirmation, preserve the index, and state explicitly that untracked content is deleted.
