...
- Open Markdown in the official DSH rendered view by default, with Preview and Source modes. Source and Markdown surfaces remain constrained to the flexible middle AppFrame track instead of deriving width from their content.
...
- Selecting a working-tree, staged, or Graph file opens that single Diff as a normal editor tab. The same Diff is deduplicated while different files and revisions can remain open together; an entire commit is never concatenated into one surface. Commit metadata stays in the main Diff toolbar instead of consuming another row for revision hashes.
...
- Keep the original DSH conversation, composer, task status, and interaction flows in the right column. File-tool paths and produced-file references rendered by DSH as clickable controls are resolved against the current Workspace and opened as normal middle-column file tabs instead of being sent to the operating system; ordinary Markdown links and code remain untouched. While the workbench is expanded, the plugin marks the actual native conversation root inside the official `conversation` slot wrapper and gives it the same official theme surface as the left sidebar; the official input card and all of its children remain untouched. Collapsing the middle editor releases both the column reorder and theme override, returning conversation to its official center layout and colors. At narrow widths, failure codes move below their full-width message. Each assistant footer is measured from its rendered content, so its clock, run time, first-token latency, and throughput move below the action icons onto a full-width wrapping line only when they would otherwise overflow. The composer toolbar remains one row by tightening gaps, hiding the reasoning-effort text and only the permission/model selector chevrons, and ellipsizing the model name. Once those chevrons are hidden, permission and model controls use equal inline padding; the model selector remains content-sized instead of consuming the toolbar's free space. The official Session-log capsule becomes a single accessible download icon only at that narrow width. The official model menu keeps its React ownership but uses viewport-fixed geometry while open, so the middle column cannot clip it.
...
- `src/workspace-backend.ts`: bounded directory, read, atomic-save, creation, rename, delete, explicit copy-path, and conversation-reference path resolution.
...
- `src/client/conversation-layout.ts`: narrow native-conversation state, content-aware assistant-metrics wrapping, responsive Session-log action marker, and clipping-safe model-menu geometry.
- `src/client/conversation-file-routing.ts`: narrow interception of DSH produced-file mentions and routing into Workspace tabs.
