...
- Browse and switch local or remote branches; create a branch from the current commit or a selected ref; rename the current branch; and safely delete merged, non-current local branches. The toolbar also shows upstream incoming/outgoing counts and exposes explicit Fetch, fast-forward Pull, Push, branch publication, and pull-then-push Sync actions.
...
- Branch and remote commands run only after explicit UI actions and disable terminal credential prompts. Branch deletion uses Git's safe merged-branch check; Pull and Sync are fast-forward only and never auto-merge, stash, or overwrite changes.
