...
- Use a dedicated toolbar icon to switch directly between Changes and Commit Graph. Its neighboring File Layout menu controls list/tree arrangement in either main view: Changes separates staged files from the working tree, while an expanded commit can arrange its files with the same choices. Each main view preserves its own layout choice.
